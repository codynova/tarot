...
const OPTIONS = {
    set (config) {
        ...
        this.cssGlobalDirectories = cssGlobalDirectories.length ? cssGlobalDirectories.map(dir => path.resolve(this.source, dir)) : [ path.resolve(this.source, 'styles') ];
        this.cssModuleDirectories = cssModuleDirectories.length ? cssModuleDirectories.map(dir => path.resolve(this.source, dir)) : [ path.resolve(this.source, 'components') ];
...
const rulesForScripts = ({ source, output, usePolyfills, useLinting, eslintConfigPath, babelConfigPath, tsConfigPath }) => ([
    !useLinting ? {} : {
        test: /\.(tsx?|jsx?)$/,
    ...
    },
    {
        test: /\.jsx?$/,
        loader: 'babel-loader',
        include: [ source, ...OPTIONS.nodeModulesToBabel ],
...
const rulesForStyles = ({ source, useCssModules, cssGlobalDirectories, cssModuleDirectories }) => useCssModules ? [
...
const pluginCopyFiles = (...pathObjects) => new PluginCopy({ patterns: pathObjects.map(({ from, to }) => ({ from: path.resolve(OPTIONS.source, from), to: path.resolve(OPTIONS.output, to) })), options: { info: true } });
...
const generateConfig = ({
    ...
    module: {
        rules: [
            ...rulesForScripts({ source, output, usePolyfills, useLinting, eslintConfigPath, babelConfigPath, tsConfigPath }),
            ...rulesForStyles({ source, useCssModules, cssGlobalDirectories, cssModuleDirectories }),
        ],
    },
    plugins: [
...
const build = ({
    ...
    try {
        ...
        return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
            ...
            options.cssGlobalDirectories = cssGlobalDirectories.length ? cssGlobalDirectories.map(dir => path.resolve(source, dir)) : OPTIONS.cssGlobalDirectories;
            options.cssModuleDirectories = cssModuleDirectories.length ? cssModuleDirectories.map(dir => path.resolve(source, dir)) : OPTIONS.cssModuleDirectories;
