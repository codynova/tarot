...
const OPTIONS = {
    set (config) {
        ...
        this.eslintFiles = eslintFiles && eslintFiles.length ? eslintFiles : null;
...
const validateConfigOptions = (raw, gen) => {
    try {
        ...
        if (raw.output && !fs.existsSync(gen.output)) {
            fs.mkdir(gen.output, { recursive: true })
        }
...
const pluginForkTypeScriptCheck = (source, eslintFiles, eslintConfigPath, tsConfigPath, tsCompilerOptions) => new PluginForkTypeScriptCheck({
    eslint: { files: eslintFiles || source, options: { configFile: eslintConfigPath, fix: true } },
...
const pluginLintScripts = ({ source, eslintFiles, eslintConfigPath, eslintIgnorePath, eslintExcludes }) => new PluginESLint({ fix: true, context: source, cwd: source, files: eslintFiles || source, overrideConfigFile: eslintConfigPath, ignorePath: eslintIgnorePath, errorOnUnmatchedPattern: false, overrideConfig: { ignorePatterns: eslintExcludes } });
...
const generateConfig = ({
    ...
    plugins: [
        ...
        useScriptLinting && useLinting ? pluginLintScripts({ source, eslintFiles, eslintConfigPath, eslintIgnorePath, eslintExcludes }) : () => {},
        useScriptLoaders ? pluginForkTypeScriptCheck(source, eslintFiles, eslintConfigPath, tsConfigPath, tsCompilerOptions) : () => {},
...
const build = ({
    ...
    return (env, argv) => {
        try {
            ...
            DEV_MODE = argv === undefined || (argv && argv.mode !== 'production');
            ...
            return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
                ...
                options.eslintFiles = eslintFiles && eslintFiles.length ? eslintFiles : OPTIONS.eslintFiles;
