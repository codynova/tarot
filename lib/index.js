...
const PluginPrettier = require('prettier-webpack-plugin');
...
const OPTIONS = {
    set (config) {
        ...
        this.eslintIgnorePath = eslintIgnorePath ? path.resolve('.', eslintIgnorePath) : path.resolve(LIB_PATH, '.eslintignore');
        this.eslintExcludes = eslintExcludes.length ? eslintExcludes.map(dir => `/${this.source}/${dir.replace(RELATIVE_FILE_PATH_REGEX, '')}`) : [];
...
const rulesForScripts = ({ source, output, usePolyfills, babelConfigPath, tsConfigPath }) => ([
    {
        test: /\.tsx?$/,
        loader: 'ts-loader',
        options: {
...
const pluginLintScripts = (source, eslintConfigPath, eslintIgnorePath, eslintExcludes) => new PluginESLint({ context: source, configFile: eslintConfigPath, ignorePath: eslintIgnorePath, ignorePattern: eslintExcludes });

const pluginPrettier = prettierConfigPath => new PluginPrettier({ configFile: prettierConfigPath });
...
const generateConfig = ({
    ...
    module: {
        rules: [
            ...rulesForScripts({ source, output, usePolyfills, babelConfigPath, tsConfigPath }),
            ...rulesForStyles({ source, useCssModules, cssGlobalIncludes, cssModuleIncludes }),
        ],
    },
    plugins: [
        useLinting ? pluginLintStyles(stylelintConfigPath) : () => {},
        useLinting ? pluginLintScripts(source, eslintConfigPath, eslintIgnorePath, eslintExcludes) : () => {},
        DEV_MODE ? pluginPrettier(prettierConfigPath) : () => {},
...
const build = ({
    ...
    try {
        ...
        return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
            ...
            options.eslintIgnorePath = eslintIgnorePath ? path.resolve('.', eslintIgnorePath) : OPTIONS.eslintIgnorePath;
            options.eslintExcludes = eslintExcludes.length ? eslintExcludes.map(dir => `/${options.source}/${dir.replace(RELATIVE_FILE_PATH_REGEX, '')}`) : OPTIONS.eslintExcludes;
