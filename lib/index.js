...
const OPTIONS = {
    set (config) {
        ...
        if (tsConfigPath) {
            ...
            tsConfigJSON.compilerOptions.lib = this.tsLib
            tsConfigJSON.include = [
                `${CWD_RELATIVE_PATH}/${relativeSource}/**/*.ts`,
                `${CWD_RELATIVE_PATH}/${relativeOutput}/**/*.tsx`,
            ];
            const aliasEntries = this.alias ? Object.entries(this.alias) : []
            if (this.alias && aliasEntries.length) {
                const paths = {}
                Object.entries(alias).forEach(([key, value]) => paths[key] = [ value.replace(RELATIVE_FILE_PATH_REGEX, '') ])
...
const rulesForScripts = ({ useScriptLoaders, source, output, usePolyfills, babelConfigPath, tsConfigPath, tsLib }) => !useScriptLoaders ? [] : [
    {
        test: /\.tsx?$/,
        loader: 'ts-loader',
        options: {
...
const pluginForkTypeScriptCheck = (source, eslintConfigPath, tsConfigPath, tsLib) => new PluginForkTypeScriptCheck({
    eslint: { files: source, options: { configFile: eslintConfigPath, fix: true } },
    typescript: { configFile: tsConfigPath, configOverwrite: { lib: tsLib } },
})
...
const generateConfig = ({
    ...
    module: {
        rules: [
            ...rulesForScripts({ useScriptLoaders, source, output, usePolyfills, babelConfigPath, tsConfigPath, tsLib }),
    ...
    plugins: [
        ...
        useScriptLoaders ? pluginForkTypeScriptCheck(source, eslintConfigPath, tsConfigPath, tsLib) : () => {},
    ...
    return (env, argv) => {
        try {
            ...
            return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
                ...
                options.tsLib = tsLib ? tsLib : OPTIONS.tsLib;
