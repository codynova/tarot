...
const OPTIONS = {
    set (config) {
        ...
        this.eslintExcludes = eslintExcludes.length ? eslintExcludes.map(dir => path.resolve(this.source, dir)) : [];
...
const build = ({
    ...
    try {
        ...
        return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
            ...
            options.eslintExcludes = eslintExcludes.length ? eslintExcludes.map(dir => path.resolve(options.source, dir)) : OPTIONS.eslintExcludes;
