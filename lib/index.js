...
const pluginCopyFiles = (...pathObjects) => new PluginCopy({ patterns: pathObjects.map(({ from, to }) => ({ from: path.resolve(OPTIONS.source, from), to: path.resolve(OPTIONS.output, to) })) });
...
const build = ({
    ...
    try {
        ...
        return Object.entries(entries).map(([ entryKey, entryConfig ]) => {
            ...
            options.cssGlobalIncludes = cssGlobalIncludes.length ? cssGlobalIncludes.map(dir => path.resolve(source, dir)) : OPTIONS.cssGlobalIncludes;
            options.cssModuleIncludes = cssModuleIncludes.length ? cssModuleIncludes.map(dir => path.resolve(source, dir)) : OPTIONS.cssModuleIncludes;
