...
const LIB_PATH = path.resolve(__filename, '..');
...
const OPTIONS = {
    set (config) {
        ...
        this.eslintConfigPath = eslintConfigPath ? path.resolve(CWD_ABSOLUTE_PATH, eslintConfigPath) : path.resolve(LIB_PATH, '.eslintrc');
        ...
        this.stylelintConfigPath = stylelintConfigPath ? path.resolve(CWD_ABSOLUTE_PATH, stylelintConfigPath) : path.resolve(LIB_PATH, '.stylelintrc');
        if (tsConfigPath) {
            ...
            fs.writeFileSync(this.tsConfigPath, JSON.stringify(tsConfigJSON));
        }
...
const pluginLintScripts = (source, eslintConfigPath, eslintIgnorePath, eslintExcludes) => new PluginESLint({ fix: true, context: source, cwd: source, configFile: eslintConfigPath, ignorePath: eslintIgnorePath, ignorePattern: eslintExcludes });
